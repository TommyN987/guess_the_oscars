    ...
    return (
        <Router>
            ...
            >
                ...
                <header className="w-full flex justify-center font-serif bg-black text-4xl opacity-90 p-2 text-gold">
                    <h1 className="z-40">Guess the Oscars</h1>
                </header>
                {user && (
                    <p className="absolute top-3 left-5 px-4 py-1 font-bold text-gold z-20">{`Welcome, ${user.name}`}</p>
                )}
                <button
                    className="absolute top-3 right-5 px-4 py-1 bg-gold text-black border-2 border-gold rounded-lg cursor-pointer hover:text-gold hover:bg-black z-20"
